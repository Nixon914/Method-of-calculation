...
def main():
    # 設置問題參數
    dim = 7  # 設計變數的數量 (b, m, z, L1, L2, d1, d2)
    # 設計變數的下界和上界
    lb = np.array([2.6, 0.7, 17, 7.3, 7.3, 2.9, 5.0])   # [b_min, m_min, z_min, L1_min, L2_min, d1_min, d2_min]
    ub = np.array([3.6, 0.8, 28, 8.3, 8.3, 3.9, 5.5])   # [b_max, m_max, z_max, L1_max, L2_max, d1_max, d2_max]
    ...
    print(f"變數範圍:")
    print(f"  2.6 ≤ b ≤ 3.6     (齒輪面寬)")
    print(f"  0.7 ≤ m ≤ 0.8     (齒輪模數)")
    print(f"  17 ≤ z ≤ 28       (小齒輪齒數, 整數)")
    print(f"  7.3 ≤ L1 ≤ 8.3    (第一軸承長度)")
    print(f"  7.3 ≤ L2 ≤ 8.3    (第二軸承長度)")
    print(f"  2.9 ≤ d1 ≤ 3.9    (第一軸直徑)")
    print(f"  5.0 ≤ d2 ≤ 5.5    (第二軸直徑)")
    print(f"\n開始優化...")
    ...
    print("\n最佳解的詳細信息:")
    print(f"  b (齒輪面寬) = {best_sol[0]:.4f}")
    print(f"  m (齒輪模數) = {best_sol[1]:.4f}")
    print(f"  z (小齒輪齒數) = {best_sol[2]:.0f}")
    print(f"  L1 (第一軸承長度) = {best_sol[3]:.4f}")
    print(f"  L2 (第二軸承長度) = {best_sol[4]:.4f}")
    print(f"  d1 (第一軸直徑) = {best_sol[5]:.4f}")
    print(f"  d2 (第二軸直徑) = {best_sol[6]:.4f}")
