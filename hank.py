...
def afsa_position_update(worst_pos, visual, lb, ub):
    ...
    for H in range(n_dim):
        r = np.random.rand()  # 随机生成一个值
        new_position = worst_pos[0,H] + visual * r
        
        # 更新Y中的第K维度的逆解
        G[0,H] = float(new_position)
        
        # 如果逆解超出边界，则随机初始化一个新的解
        if G[0,H] < lb or G[0,H] > ub:
            G[0,H] = np.random.uniform(lb, ub)  # 在边界范围内随机生成新解
    return G
...
def test_gwo_afsa_on_cec2017():
    ...
    for i, f in enumerate(functions.all_functions):
        ...
        for run in range(runs):
            best_fitness,kqc,worstc,sac,fitness_countera,fitness_counterb,fitness_counterc = gwo_afsa_with_worst_update(f, dim, lb, ub, n_wolves, max_iter)
