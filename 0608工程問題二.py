...
def gwo_afsa_with_worst_update(f, dim, lb, ub, n_wolves, max_iter):
    ...
    for t in range(max_iter):
        ...
        for i in range(n_wolves):
            ...
            for idx in worst_indices:
                ...
                levy_step = levy_flight(dim, sigma=0.01)  # 適當調整sigma值
    ...
    print(f"  g2 (挫屈) = {g2:.6f} {'✓' if g2 <= 0 else '✗'}")
    print(f"  g3 (撓度) = {g3:.6f} {'✓' if g3 <= 0 else '✗'}")
    print(f"  g4 (空間) = {g4:.6f} {'✓' if g4 <= 0 else '✗'}")
...
def main():
    ...
    dim = 3  # 設計變數的數量: [d, D, N]
    # 設計變數的下界和上界
    lb = np.array([0.05, 0.25, 2.0])   # [線徑, 彈簧平均直徑, 活動圈數]
