...
def main():
    # 設置問題參數
    dim = 4  # 設計變數的數量 (h, l, t, b)
    # 設計變數的下界和上界
    lb = np.array([0.1, 0.1, 0.1, 0.1])   # [h_min, l_min, t_min, b_min]
    ub = np.array([2.0, 10.0, 10.0, 2.0])    # [h_max, l_max, t_max, b_max]
    ...
    print("設計變數: [h, l, t, b]")
    print(f"變數範圍:")
    print(f"  0.1 ≤ h ≤ 2.0    (焊縫厚度)")
    print(f"  0.1 ≤ l ≤ 10.0   (連接部分長度)")
    print(f"  0.1 ≤ t ≤ 10.0   (梁的高度)")
    print(f"  0.1 ≤ b ≤ 2.0    (梁的厚度)")
    print(f"\n開始優化...")
    ...
    for i in range(30):
        ...
        print(f"運行 {i+1}/30 完成: 最佳成本 = {best_cost:.6f}, 執行時間 = {execution_time:.2f} 秒")
    ...
    print("\n===== 總統計結果 =====")
    print(f"最佳成本: {best_cost_overall:.6f}")
    print(f"最差成本: {worst_cost_overall:.6f}")
    print(f"平均成本: {mean_cost:.6f}")
    print(f"中位數成本: {median_cost:.6f}")
    ...
    print("\n最佳解的詳細信息:")
    print(f"  h (焊縫厚度) = {best_sol[0]:.6f}")
    print(f"  l (連接部分長度) = {best_sol[1]:.6f}")
    print(f"  t (梁的高度) = {best_sol[2]:.6f}")
    print(f"  b (梁的厚度) = {best_sol[3]:.6f}")
    ...
    print("\n最佳解約束條件檢查:")
    print(f"  g1 (剪應力) = {g1:.6f} {'✓' if g1 <= 0 else '✗'}")
    print(f"  g2 (正應力) = {g2:.6f} {'✓' if g2 <= 0 else '✗'}")
    print(f"  g3 (偏轉量) = {g3:.6f} {'✓' if g3 <= 0 else '✗'}")
    print(f"  g4 (厚度關係) = {g4:.6f} {'✓' if g4 <= 0 else '✗'}")
    print(f"  g5 (臨界載荷) = {g5:.6f} {'✓' if g5 <= 0 else '✗'}")
    print(f"  g6 (焊縫厚度下限) = {g6:.6f} {'✓' if g6 <= 0 else '✗'}")
    ...
    plt.axhline(y=mean_cost, color='r', linestyle='-', label=f'平均值: {mean_cost:.6f}')
    plt.axhline(y=best_cost_overall, color='g', linestyle='-', label=f'最佳值: {best_cost_overall:.6f}')
