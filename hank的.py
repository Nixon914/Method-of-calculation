...
output_file = f"演算法_{timestamp}.txt"
fitness_counter = 0
# 模拟退火局部优化函数
...
def gwo_afsa_with_worst_update(f, dim, lb, ub, n_wolves, max_iter):
    ...
    convergence_curve=np.zeros((1,max_iter))# initialization fusion curve
    ...
    for t in range(max_iter):
        ...
        for i in range(n_wolves):
            ...
            for idx in worst_indices:
                new_pos = afsa_position_update(X[idx, :], visual=1, lb=lb, ub=ub)  # 獲取新位置
...
def write_results_to_file(output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for i, func in enumerate(functions.all_functions):
            function_results = []
            f.write(f"\nFunction f{i+1} Results:\n")
            f.write("-" * 30 + "\n")
            
            total_fitness_counter = 0
            total_kqc = 0
            total_sac = 0
            total_fitness_counter_OUT = 0
            total_worstc = 0
            total_reverse_fitness_c = 0
            
            # Execute multiple test runs
            for run in range(runs):
                best_fitness, fitness_counter, kqc, sac, fitness_counter_OUT, worstc, reverse_fitness_c = \
                    gwo_afsa_with_worst_update(func, dim, lb, ub, n_wolves, max_iter)
                function_results.append(best_fitness)
                
                # Write individual run results
                f.write(f"\nRun {run + 1}:\n")
                f.write(f"Best Fitness: {best_fitness}\n")
                f.write(f"Fitness Counter: {fitness_counter}\n")
                f.write(f"Quadratic Interpolation Counter: {kqc}\n")
                f.write(f"Simulated Annealing Counter: {sac}\n")
                f.write(f"Fitness Counter OUT: {fitness_counter_OUT}\n")
                f.write(f"Worst Update Counter: {worstc}\n")
                f.write(f"Reverse Learning Counter: {reverse_fitness_c}\n")
                
                # Accumulate counters
                total_fitness_counter += fitness_counter
                total_kqc += kqc
                total_sac += sac
                total_fitness_counter_OUT += fitness_counter_OUT
                total_worstc += worstc
                total_reverse_fitness_c += reverse_fitness_c
            
            # Calculate statistics
            best_score = np.min(function_results)
            mean_score = np.mean(function_results)
            std_dev = np.std(function_results)
            
            # Write summary statistics
            f.write("\nFunction Summary Statistics:\n")
            f.write(f"Best Fitness: {best_score}\n")
            f.write(f"Mean Fitness: {mean_score}\n")
            f.write(f"Standard Deviation: {std_dev}\n")
            f.write(f"Average Fitness Counter: {total_fitness_counter/runs}\n")
            f.write(f"Average Quadratic Interpolation: {total_kqc/runs}\n")
            f.write(f"Average Simulated Annealing: {total_sac/runs}\n")
            f.write(f"Average Fitness Counter OUT: {total_fitness_counter_OUT/runs}\n")
            f.write(f"Average Worst Update: {total_worstc/runs}\n")
            f.write(f"Average Reverse Learning: {total_reverse_fitness_c/runs}\n")
            f.write("\n" + "=" * 50 + "\n")
# 运行测试
